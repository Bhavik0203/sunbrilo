...
import { newsletters, getNewsletterBySlug } from '../../lib/newsletterData';
...
export default function NewsletterDetailPage() {
  ...
  const post = getNewsletterBySlug(params.slug);
        ...
        <section className="mx-auto mt-8 w-full max-w-7xl">
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                ...
                <div className="rounded-2xl bg-[#f7f5ef] p-5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          window.location.href = `/newsletters?search=${encodeURIComponent(e.currentTarget.value)}`;
                        }
                      }}
                    />
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {topics.map((t) => (
                        <Link
                          key={t}
                          href={`/newsletters?topic=${encodeURIComponent(t)}`}
            ...
            <main className="lg:col-span-9">
              <article className="rounded-2xl bg-white p-6 md:p-10 shadow-sm">
                ...
                <div className="space-y-6">
                  {paragraphs.map((para, i) => {
                    // Check if it is a list keypoint: e.g. "Supply Chain Optimization: AI can..."
                    if (para.includes(': ') && !para.startsWith('http') && para.length < 350) {
                      const colonIndex = para.indexOf(': ');
                      const label = para.slice(0, colonIndex);
                      const detail = para.slice(colonIndex + 1);
                      return (
                        <div key={i} className="flex gap-3 leading-relaxed text-[#4b4b4b] font-raleway text-[15px] pl-2 border-l-2 border-gray-100 hover:border-[#ffee50] transition-colors py-1">
                          <span className="text-[#ffee50] font-bold text-lg select-none">•</span>
                          <p>
                            <strong className="text-[#3b3808] font-bold">{label}</strong>: {detail.trim()}
                          </p>
                        </div>
                      );
                    }
                ...
                {/* Styled Premium CTA Banner */}
                <div className="mt-12 p-6 md:p-8 rounded-2xl bg-gradient-to-br from-[#3B3808] to-[#1d1b04] text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-md border border-[#ffee50]/10">
                  <div className="max-w-xl">
                    <h3 className="text-xl font-bold text-[#ffee50] font-raleway">Ready to capitalize on this?</h3>
                    <p className="text-sm text-gray-300 mt-2 font-raleway">
                  ...
                  <Link
                    href={post.ctaLink}
                    className="inline-flex items-center gap-2 rounded-full bg-[#ffee50] px-6 py-3.5 text-sm font-semibold text-[#3B3808] hover:bg-white hover:text-[#3B3808] transition-all font-raleway shadow-lg shadow-[#ffee50]/10 shrink-0"
                  >
                    {post.ctaText}
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="mt-10 pt-6 border-t border-black/10 flex flex-wrap gap-2">
                  {[post.tag, 'Enterprise', 'Insights'].map((tag) => (
                ...
                <div className="mt-8 flex items-center gap-4 rounded-2xl bg-[#f7f5ef] p-5">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#6b6b6b] mt-0.5 font-raleway">
                      Expert insights for enterprises driving growth through digital capitalization.
                    </div>
                  </div>
                </div>
              ...
              {/* ── Related Articles ───────────────────────────────────── */}
              <div className="mt-8">
                <h2 className="text-xl font-bold text-[#1b1b1b] mb-4 font-raleway">Related Newsletters</h2>
